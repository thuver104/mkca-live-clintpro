...
export default function NewFormPage() {
  ...
  const updateField = (idx: number, key: keyof Field, val: string | boolean) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await fetch("/api/forms", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    router.push("/admin/forms");
  };

  return (
    <div className="max-w-2xl">
      <h1 className="font-heading text-3xl font-bold text-chess-100 mb-8">New Registration Form</h1>
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="border-t border-chess-700/30 pt-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button type="button" onClick={addField} className="text-sm text-chess-accent hover:text-chess-accentHover transition-colors">
              <i className="fas fa-plus mr-1"></i>Add Field
            </button>
          </div>

          {fields.length === 0 && <p className="text-chess-100/40 text-sm text-center py-6">No fields added yet. Click &quot;Add Field&quot; to start.</p>}
          ...
          <div className="space-y-4">
            {fields.map((field, idx) => (
              <div key={field.id} className="bg-chess-800/30 border border-chess-700/30 rounded-xl p-4 space-y-3">
                ...
                <input type="text" value={field.placeholder} onChange={(e) => updateField(idx, "placeholder", e.target.value)} placeholder="Placeholder text"
                ...
                {field.type === "select" && (
                  <input type="text" value={field.options} onChange={(e) => updateField(idx, "options", e.target.value)} placeholder="Options (comma-separated)"
                    className="w-full bg-chess-800/60 border border-chess-700/50 rounded-lg px-3 py-2 text-chess-100 text-sm placeholder-chess-100/30 focus:outline-none focus:border-chess-accent/50" />
                )}
